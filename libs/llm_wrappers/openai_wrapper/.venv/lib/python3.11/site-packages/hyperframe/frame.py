...
class Frame:
    ...
    def __init__(self, stream_id: int, flags: Iterable[str] = ()) -> None:
        ...
        if self.stream_id and self.stream_association == _STREAM_ASSOC_NO_STREAM:
            msg = (
                f"Stream ID must be zero for {type(self).__name__} with stream_id={self.stream_id}"
            )
            raise InvalidDataError(msg)

    def __repr__(self) -> str:
        return f"{type(self).__name__}(stream_id={self.stream_id}, flags={self.flags!r}): {self._body_repr()}"
...
class SettingsFrame(Frame):
    ...
    def __init__(
        self, stream_id: int = 0, settings: dict[int, int] | None = None, **kwargs: Any
    ) -> None:
        super().__init__(stream_id, **kwargs)
    ...
    def serialize_body(self) -> bytes:
        return b"".join(
            [_STRUCT_HL.pack(setting & 0xFF, value) for setting, value in self.settings.items()]
        )
...
class PushPromiseFrame(Padding, Frame):
    ...
    def __init__(
        self, stream_id: int, promised_stream_id: int = 0, data: bytes = b"", **kwargs: Any
    ) -> None:
        super().__init__(stream_id, **kwargs)
    ...
    def parse_body(self, data: memoryview) -> None:
        ...
        self.data = data[padding_data_length + 4 : len(data) - self.pad_length].tobytes()
        self.body_len = len(data)
...
class ExtensionFrame(Frame):
    ...
    def __init__(
        self, type: int, stream_id: int, flag_byte: int = 0x0, body: bytes = b"", **kwargs: Any
    ) -> None:  # noqa: A002
        super().__init__(stream_id, **kwargs)
        self.type = type
