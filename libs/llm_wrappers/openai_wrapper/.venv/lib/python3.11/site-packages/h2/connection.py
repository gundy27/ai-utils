...
class H2ConnectionStateMachine:
    ...
    _transitions = {
        ...
        (ConnectionState.IDLE, ConnectionInputs.SEND_PRIORITY): (None, ConnectionState.IDLE),
        (ConnectionState.IDLE, ConnectionInputs.RECV_PRIORITY): (None, ConnectionState.IDLE),
        ...
        # State: closed
        (ConnectionState.CLOSED, ConnectionInputs.SEND_GOAWAY): (None, ConnectionState.CLOSED),
        (ConnectionState.CLOSED, ConnectionInputs.RECV_GOAWAY): (None, ConnectionState.CLOSED),
    }
...
class H2Connection:
    ...
    def _begin_new_stream(self, stream_id: int, allowed_ids: AllowedStreamIDs) -> H2Stream:
        ...
        highest_stream_id = (
            self.highest_outbound_stream_id if outbound else self.highest_inbound_stream_id
        )
    ...
    def send_headers(
        ...
        if stream_id not in self.streams:
            ...
            )  # take a copy due to the property accessor having side affects
    ...
    def push_stream(
        self, stream_id: int, promised_stream_id: int, request_headers: Iterable[HeaderWeaklyTyped]
    ) -> None:
        """
    ...
    def advertise_alternative_service(
        self, field_value: bytes | str, origin: bytes | None = None, stream_id: int | None = None
    ) -> None:
        """
    ...
    def _handle_data_on_closed_stream(
        ...
        if conn_increment:
            ...
            self.config.logger.debug(
                "Received DATA frame on closed stream %d - " "auto-emitted a WINDOW_UPDATE by %d",
                frame.stream_id,
                conn_increment,
            )
